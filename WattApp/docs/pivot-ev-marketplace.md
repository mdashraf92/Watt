...
**Scope:** `mobile-app/` (Expo app), `mobile-app/backend/` (Node/Postgres API), and the marketing site
...
booking (`mobile-app/src/types/index.ts`). Sessions bill by meter reading, with a wallet hold
...
### Backend modules (`mobile-app/backend/src/modules/`)
...
### App screens (`mobile-app/src/screens/`)
...
(`mobile-app/src/types/index.ts`) and in the auth middleware, so this is a contained change.
...
| Area | Files |
| --- | --- |
| Pricing model | `mobile-app/src/types/index.ts` (`price_per_kwh` on `Station`, `ChargerListing`, `ChargingSession`) |
| Session billing | `backend/src/modules/sessions/`, `backend/sql/backend-billing-overrun-fix.sql`, `backend-overstay-and-refund.sql` |
| Map | `mobile-app/src/screens/MapScreen.tsx`, `src/components/MapFilterSheet.tsx` *(uncommitted)*, `src/lib/listingToStation.ts` |
| Booking → packages | `mobile-app/src/screens/BookingScreen.tsx`, `backend/src/modules/bookings/bookings.routes.ts` |
...
| Roles | `mobile-app/src/types/index.ts` (`Profile.role`), auth middleware |
| Copy | `mobile-app/src/i18n/ar.ts`, `mobile-app/src/i18n/en.ts` — always in the same commit |

---

**A note for whoever implements this:** `AGENTS.md` at the app root requires reading the
exact Expo v57 docs (https://docs.expo.dev/versions/v57.0.0/) before writing app code.
